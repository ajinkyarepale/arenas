...
export async function GET(
  ...
  // Viewer-selected timeframe, restricted to what Binance klines genuinely
  // serve. Anything else falls back to the round-matched interval — a
  // nonfunctional option is never presented as working.
  const SUPPORTED = new Set(['1m', '3m', '5m', '15m', '30m', '1h']);
  const requested = url.searchParams.get('interval');
  const interval =
    requested && SUPPORTED.has(requested)
      ? requested
