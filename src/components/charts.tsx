  ...
  return (
    ...
    >
      ...
      <path
        ...
        strokeLinecap="round"
        vectorEffect="non-scaling-stroke"
        style={{ filter: `drop-shadow(0 0 6px ${up ? 'rgba(0,232,150,0.5)' : 'rgba(255,61,100,0.5)'})` }}
      />
      {/* Emphasised endpoint — where the trader actually stands. */}
      <circle
        ...
        fill={stroke}
        vectorEffect="non-scaling-stroke"
        style={{ filter: `drop-shadow(0 0 6px ${up ? 'rgba(0,232,150,0.7)' : 'rgba(255,61,100,0.7)'})` }}
      />
    </svg>
  );
}
...
export function CalibrationChart({
  ...
  return (
    ...
    >
      ...
      {/* One dot per bucket, area scaled by how many trades it holds. */}
      {populated.map((b) => {
      ...
      {/* Axis labels */}
      <text x={inset} y={height - 8} fill="#8a919e" fontSize="9" fontFamily="var(--font-mono)">
        0%
      </text>
      <text
        x={width - inset}
        ...
        {rounds.map((r) => {
          ...
          return (
            <div key={r.roundNumber} className="flex min-w-[2.5rem] flex-1 flex-col gap-1">
              {/* Market belief as a vertical YES/NO split */}
              <div
                className="relative h-20 overflow-hidden rounded-sm border border-line bg-ink-900"
                title={`Round ${r.roundNumber}: market said ${formatProbability(implied)} YES, resolved ${r.outcome ?? '—'}`}
              >
                <div
                  className="absolute inset-x-0 bottom-0 bg-yes/25"
                  style={{ height: `${implied * 100}%` }}
                />
                {/* 50% reference */}
              ...
              <div
                className={cx(
                  'rounded-sm py-0.5 text-center font-mono text-[10px] font-bold',
                  isVoid
              ...
              <div
                className={cx(
                  ...
                  r.marketCorrect === true
                    ? 'text-yes'
                    : r.marketCorrect === false
                      ? 'text-no'
                      : 'text-fg-faint',
                )}
                title={
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-ink-800">
        <div className="bg-yes/70" style={{ width: `${pct}%` }} />
