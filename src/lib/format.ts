...
// ---------------------------------------------------------------------------
// Arcs (ARC) — virtual credits. Plain-text fallback uses the "ARC" prefix
// (the Ɐ glyph is unreliable across system fonts); visual UI must use the
// <ArcsMark /> SVG via <Arcs /> instead.
// ---------------------------------------------------------------------------

/** ARC 12,450 — plain-text Arcs formatting. */
export function formatArcs(value: number, decimals = 2): string {
  return `ARC ${formatPoints(value, decimals)}`;
}

/** +ARC 350 / −ARC 120 — plain-text signed Arcs for P&L. */
export function formatSignedArcs(value: number, decimals = 2): string {
  const sign = value > 0 ? '+' : value < 0 ? '−' : '';
  return `${sign}ARC ${formatPoints(Math.abs(value), decimals)}`;
}

/** IST (Asia/Kolkata) 12-hour clock, e.g. "4:32:05 PM". Display-layer only. */
export function formatTimeIST(ms: number, withSeconds = true): string {
...
/** IST date + 12-hour time, e.g. "25 Sep, 4:32 PM". Display-layer only. */
export function formatDateTimeIST(ms: number): string {
  return new Intl.DateTimeFormat('en-IN', {
    timeZone: 'Asia/Kolkata',
    day: 'numeric',
    month: 'short',
